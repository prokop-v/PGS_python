...
Global Variables
"""
# First index = count of mined blocks/sources
# Second index = average time per block/source
blocks_mined = [0, 0]
sources_mined = [0, 0]
...
def parse_file(log_file_path):
    """
    Function to search the input file for data,
    and processes valuable data
    :param log_file_path: Input file
    :return: worker_actions, lorry_actions, ferry_actions, simulation_t
    """

    # here we will store found data
    worker_actions = []
    lorry_actions = []
    ferry_actions = [0, 0]
    ...
    with open(log_file_path, "r", encoding="utf-8") as file:
        # to load, when simulation starts
        first_line = True
        ...
        for line in file:
            ...
            if first_line:
                start_time_simulation = parts[0] + " " + parts[1]  # store the date
                first_line = False
            ...
            if WORKER_MINED_A_SOURCE in line:
                ...
                if action_id not in [action['id'] for action in worker_actions]:
                    ...
                    # else find id in list and add source
            ...
            if W_INTO_LORRY in line:
                action_id = parts[4]
                worker_index = [action['id'] for action in worker_actions].index(action_id)
                # last thing workers do, we will recalculate to last put to know when worker has ended
            ...
            # adding transport time to lorries
            ...
            # another transport time for lorries, when driving to the end
            if LORRY_AT_THE_END in line:
                action_id = parts[4]
                lorry_index = [action['id'] for action in lorry_actions].index(action_id)
                lorry_actions[lorry_index]["transport_time"] += int(parts[12])
                # we also know that this is the time,
                # when last lorry ends its journey and simulation ends
                end_time = parts[0] + " " + parts[1]

            # working with ferry
            if FERRY_DEPARTURE in line:
                ferry_actions[0] += 1  # 1 for each trip
                ferry_actions[1] += int(parts[8])  # how long that trip lasts
...
def time(start_time, end_time):
    """
    Function that calculates the time between two datetime objects,
    even with days, months and years
    :param start_time: t1
    :param end_time: t2
    :return: time in ms
    """
...
def generate_xml(worker_actions, lorry_actions, ferry_actions, simulation_t, output_file_path):
    """
    Function that generates the XML structure
    :param worker_actions: worker actions
    :param lorry_actions: lorry actions
    :param ferry_actions: ferry actions
    :param simulation_t: length of the simulation
    :param output_file_path: file to output the XML
    """
    ...
    for worker_action in sorted_worker_actions:
        ...
        worker_duration_element = ET.SubElement(worker_element, "workDuration")
        worker_duration_element.text = str(worker_action["work_time"]) + " ms"
    ...
    for lorry_action in sorted_lorry_actions:
        ...
        load_time_element = ET.SubElement(lorry_element, "loadTime")
        load_time_element.text = str(lorry_action["time_to_fill"]) + " ms"
        transport_time_element = ET.SubElement(lorry_element, "transportTime")
        transport_time_element.text = str((lorry_action["transport_time"] + average_wait_time)) + " ms"
...
def main():
    """
    Main function of the program that will parse the command line arguments,
    a run generate XML function
    """
    ...
    # parse file
    worker_actions, lorry_actions, ferry_actions, simulation_t = parse_file(args.input)
